...
1.解析：回溯
    ...
    1）从nums首元素遍历到尾元素。对当前元素，有添加/不添加2种情况，对应是否保存到一维数组temp.
...
public:
    ...
    void dfs(vector<int>& nums, int index, vector<vector<int>>& res, vector<int>& vec) {
        if (index == nums.size()) {
            res.push_back(vec); // push_back()操作的是数据副本, 而非引用
            return;
        }
        // 情况一: 不含当前元素
        dfs(nums, index + 1, res, vec); 

        // 情况二: 包含当前元素
        vec.push_back(nums[index]);
        dfs(nums, index + 1, res, vec);
        vec.pop_back(); // 该写法为回溯标准写法
        // vec.erase(vec.end() - 1); // 该写法可行但不推荐
    }
};
```
